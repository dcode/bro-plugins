...
void KafkaWriter::InitConfigOptions()
{
    ...
    meta_json.assign(
        (const char*) BifConst::Kafka::meta_json->Bytes(),
        BifConst::Kafka::meta_json->Len());
...
bool KafkaWriter::DoInit(const WriterInfo& info, int num_fields, const threading::Field* const* fields)
{
    ...
    if( strcmp(json_format.c_str(), "Kafka::JS_DEFAULT") == 0 )
        ...
        formatter = new threading::formatter::FlexiJSON(info.path, this, tf, meta_json);
    else
    {
...
bool KafkaWriter::DoFinish(double network_time)
{
    ...
    for( int i=0; i < dedot_numfields; i++ )
    {
        delete dedot_fields[i];
    }
    free( dedot_fields );
